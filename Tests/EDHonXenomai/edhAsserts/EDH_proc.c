...
int IsResume = 1;
static dynamic_policy current_policy;
_Bool entrance = true;
...
int Scheduler_create_task(Input_task input_task[MAX_TASKS_NUMBER],int nb)
{
  ...
  for(int i=0;i<nb;i++){
    ...
    /*Template :
        rt_task_create_dyna, (RT_TASK *task, const char *name,
			      int stksize, xnticks_t next_deadline,double WCET,double WCEC,
                              dyna_policy policy, int mode)
    */

    if (Scheduler_get_policy() == EDF){
        if(rt_task_create_dyna(&loop_task[i], str, 0, input_task[i].deadline,0,0,EDF,0)!=0){
          printf("rt_task_create_dyna error\n");
          return 1;
        }
    }else if (Scheduler_get_policy() == EDH_ASAP){
      //TODO
    }else if (Scheduler_get_policy() == EDH_ASAP){
      //TODO
    }else{
...
void Scheduler_set_policy(dynamic_policy policy){
	current_policy = policy;
}

dynamic_policy Scheduler_get_policy(){
